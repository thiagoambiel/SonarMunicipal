...
O `core` é o coração algorítmico do Sonar Municipal: um pacote Python enxuto e reutilizável que
implementa a busca semântica, o cálculo de efeitos sobre indicadores e o agrupamento de Projetos
de Lei em políticas públicas. A plataforma web reproduz a mesma lógica em TypeScript; este módulo
é a referência canônica e o ponto de entrada para usar o algoritmo fora da aplicação (notebooks,
scripts, avaliação).
...
- [Formato esperado do dataset](#formato-esperado-do-dataset)
- [Funções principais](#funções-principais)
- [Exemplo: busca semântica](#exemplo-busca-semântica)
- [Exemplo: efeitos e políticas](#exemplo-efeitos-e-políticas)
...
- Agrupar PLs semelhantes em políticas, ordenadas por critérios de qualidade.
...
O arquivo `dataset.npy` é produzido conforme o `DATASET.MD` (caminho rápido: baixar o dataset
publicado no Hugging Face e gerar os embeddings). Ele deve conter uma lista de dicionários com,
no mínimo:
- `municipio` (str)
- `uf` (str)
...
> **Normalização de município (contrato com indicadores).** `compute_effects_from_indicator`
> casa `municipio` com o indicador usando apenas `.upper()` — **não remove acentos**. Portanto a
> coluna-chave do CSV de indicador (`city_col`) deve estar na mesma forma acentuada e em caixa
> alta do campo `municipio` do dataset. É o caso de `indicators/homicidios.csv` (coluna
> `municipio_norm`). CSVs sem acento (ex.: a coluna `municipio` de `matriculas.csv`) não casam por
> essa rota.
